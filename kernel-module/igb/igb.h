...
/* AVB SPECIFIC DEFINES */

#define MAX_AVB_STREAMS 32
#define MAX_SUBSTREAMS 1

/* Commands send to the driver through a write call */

#define TALKER_CONNECT		0
#define LISTENER_CONNECT	1
#define TALKER_DISCONNECT	2
#define LISTENER_DISCONNECT	3
#define SAMPLERATE_CHANGE	4

struct avb_input_stream {
        uint64_t stream_id;
        uint8_t sequence_nr;
        uint8_t channels;
        uint8_t dbc_continuity;
        uint16_t samplerate;
	};

struct avb_output_stream {
        uint64_t stream_id;
        uint64_t dest_mac;
        uint16_t vlan;
        uint8_t sequence_nr;
...
struct igb_adapter {
	...
	wait_queue_head_t	wq;
        ...
        void *addr;  // kernel address of ring/dma area
        dma_addr_t physaddr;  // physical address of ring/dma area
        uint32_t size;

/* alsa specific */

        struct avb_input_stream avb_input_streams[MAX_AVB_STREAMS];
        struct avb_output_stream avb_output_streams[MAX_AVB_STREAMS];

        void *card;  // struct snd_card *card
        void *pcm;    // struct snd_pcm *pcm;

        int rate; /* sample rate */
        int samples_per_packet; /* num samples of a channel in an ethernet packet */
        int playback_channels; /* playback channels */
        int playback_period_size; /* playback period size */
        int playback_buffer_size; /* playback buffer size */
        int hw_playback_pointer; /* hardware pointer */
        int playback;  /* playback is active */
        int p_cnt;  /* samples transmitted since last period_elapsed */

        int capture_channels; /* capture channels */
        int capture_period_size; /* playback period size */
        int capture_buffer_size; /* playback buffer size */
        ...
        void *rx_addr;  // kernel address of rx network buffer
        dma_addr_t rx_physaddr;  // physical address of rx network buffer

        //void *p_addr;  // kernel address of tx network buffer
        //dma_addr_t p_physaddr;  // physical address of tx network buffer

        //void *c_addr;  // kernel address of rx network buffer
        //dma_addr_t c_physaddr;  // physical address of rx network buffer
...
/* set of Drumfix ioctl calls */
#define IGB_IOCTL_MCAST_BASE			_IOW('E', 401, int)
#define IGB_IOCTL_SET_SYNC_STREAM		_IOW('E', 401, int)
#define IGB_IOCTL_CLR_SYNC_STREAM		_IOW('E', 402, int)
#define IGB_IOCTL_TALKER_JOIN			_IOW('E', 403, int)
#define IGB_IOCTL_TALKER_LEAVE			_IOW('E', 404, int)
#define IGB_IOCTL_LISTENER_JOIN			_IOW('E', 405, int)
#define IGB_IOCTL_LISTENER_LEAVE		_IOW('E', 406, int)
...
struct igb_avb_mcast_base {
    char mcast_base[6];
};

struct igb_avb_sync_stream {
    char stream_id[8];
};

struct igb_avb_talker_listener {
    char id[8];             /* mac address + unique address */
    char mcast_address[6];  /* multicast address of the stream */
};
