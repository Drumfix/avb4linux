 ...
 * Standard best-effort LAN traffic is directed to tx queue(3), leaving
 * tx-queue(0) through tx-queue(2) on the Intel i210 available to be mapped
 * into a user-space application and managed by the application.
 *
 ...
 * can use. In this current form, the driver assumes all receive
 * packets will go to the same queue shared with the best-effort LAN
 * traffic - although someone can customize this if they desire
 * to filter specific traffic to other receive queues (see ETFQ and
 * similar registers for details).
 */
...
static int samplerate = 48000; /*default value*/
...
static void igb_assign_vector(struct igb_q_vector *q_vector, int msix_vector)
{
	...
	case e1000_i211:
		...
		if (tx_queue > IGB_N0_QUEUE)
            ...
            if (tx_queue)  //DRUMFIX, do not enable TXRX0 interrupt */
		   q_vector->eims_value = 1 << msix_vector;
		break;
	default:
		BUG();
	...
	for (i = 0; i < adapter->num_q_vectors; i++) {
		...
		if (q_vector->rx.ring && q_vector->tx.ring)
			...
			sprintf(q_vector->name, "%s-unused", netdev->name);
                ...
                /* DRUMFIX do not use threaded irqs */
		err = request_irq(adapter->msix_entries[vector].vector,
...
//        snd_avb_probe(adapter);

	adapter->last_event = 0;
	adapter->event = 0;
	init_waitqueue_head(&adapter->wq);
        adapter->addr = NULL;
        adapter->physaddr = 0;
        adapter->size = 0;
/* #### ralf end #### */
...
static int __igb_open(struct net_device *netdev, bool resuming)
{
	...
	igb_configure(adapter);
        ...
        err = snd_avb_probe(adapter, samplerate);
	if (err)
		goto err_setup_rx;
	...
	schedule_work(&adapter->watchdog_task);
        ...
        adapter->q_vector[0]->itr_val = 4;
        adapter->q_vector[0]->set_itr = 1;
        igb_write_itr(adapter->q_vector[0]);
...
static void igb_watchdog_task(struct work_struct *work)
{
	...
	igb_update_stats(adapter);
        ...
        /* DRUMFIX do not do this for the AVB queues */
 
	for (i = 2; i < adapter->num_tx_queues; i++) {
	...
	if (adapter->msix_entries) {
		u32 eics = 0;
                ...
                /* DRUMFIX do not for AVB queues */
		for (i = 2; i < adapter->num_q_vectors; i++)
...
/* #### ralf start #### */
	...
	/* service user (AVB) queues */
	if (rx_ring->queue_index == 1)
        {
                snd_avb_receive(adapter);
		E1000_WRITE_REG(hw, E1000_EIMS, q_vector->eims_value);

                return IRQ_HANDLED;
//		adapter->event++;
//		wake_up_interruptible(&adapter->wq);
        }
        else if (rx_ring->queue_index == 0)
        {
		E1000_WRITE_REG(hw, E1000_EIMS, q_vector->eims_value);
		return IRQ_HANDLED;
        }
...
/* ralf: TODO: ignore receive errors
		if (igb_test_staterr(rx_desc,
...
static void igb_netpoll(struct net_device *netdev)
{
	...
	int i;
        ...
        /* DRUMFIX not do this for AVB queues */
	for (i = 2; i < adapter->num_q_vectors; i++) {
		q_vector = adapter->q_vector[i];
		if (adapter->msix_entries)
			...
			loff_t *pos)
{
	...
	while (adapter->last_event == adapter->event)
	{
		DEFINE_WAIT(wait);
		...
		prepare_to_wait(&adapter->wq, &wait, TASK_INTERRUPTIBLE);

		if (adapter->last_event == adapter->event)
		{
			schedule();
		}

		finish_wait(&adapter->wq, &wait);
	...
	adapter->last_event = adapter->event;

	copy_to_user(buf, &adapter->last_event, sizeof(adapter->last_event));

	return sizeof(adapter->last_event);
}
			 ...
			 size_t count, loff_t *pos)
{
	...
	if (adapter == NULL)
        ...
        if (copy_from_user(&cmd, buf, sizeof(Avb_Cmd))) {
           return -EFAULT;
        }
        ...
        switch (cmd.cmd)
        {
           case TALKER_CONNECT:
           {
              if (cmd.substream >= MAX_SUBSTREAMS)
                 return -EINVAL;

              if (cmd.block >= MAX_AVB_STREAMS_PER_SUBSTREAM)
                 return -EINVAL;

              if (cmd.index >= MAX_AVB_STREAMS)
                 return -EINVAL;

              adapter->avb_input_streams[cmd.index].stream_id = cmd.stream_id;

              //adapter->alsa_input_substreams[cmd.substream].block[cmd.block] = cmd.stream_id;
           ...
           case LISTENER_CONNECT:
           {
              ...
              adapter->avb_output_streams[cmd.index].stream_id = cmd.stream_id;
              adapter->avb_output_streams[cmd.index].dest_mac = cmd.dest_mac;
              adapter->avb_output_streams[cmd.index].vlan = cmd.vlan;

              //adapter->alsa_output_substreams[cmd.substream].block[cmd.block] = cmd.stream_id;

              break;
           }
           case TALKER_DISCONNECT:
           {
              if (cmd.substream >= MAX_SUBSTREAMS)
                 return -EINVAL;
              ...
              adapter->avb_input_streams[cmd.index].stream_id = 0;
              //adapter->alsa_input_substreams[cmd.substream].block[cmd.block] = 0;

              break;
           }
           case LISTENER_DISCONNECT:
           {
              if (cmd.substream >= MAX_SUBSTREAMS)
                 return -EINVAL;
              ...
              adapter->avb_output_streams[cmd.index].stream_id = 0;
              adapter->avb_output_streams[cmd.index].dest_mac = 0;
              adapter->avb_output_streams[cmd.index].vlan = 0;

              //adapter->alsa_output_substreams[cmd.substream].block[cmd.block] = 0;
...
{
/*
        struct igb_private_data *igb_priv = vma->vm_private_data;
	struct igb_adapter *adapter = igb_priv->adapter;

        if (adapter != NULL && adapter->addr != NULL && adapter->physaddr != 0)
		dma_free_coherent(&adapter->pdev->dev, adapter->size, adapter->addr, adapter->physaddr);
...
static int igb_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (pgoff == 0)
		...
		if (remap_pfn_range(vma, vma->vm_start, physaddr, size, vma->vm_page_prot))
			return -ENODEV;
        }
	else
//		physaddr = pgoff; 
        {
                adapter->addr = dma_alloc_coherent(&adapter->pdev->dev, size, &adapter->physaddr, GFP_KERNEL);
        
                if (adapter->addr == NULL)
                   return -EFAULT;

		if (remap_pfn_range(vma, vma->vm_start, adapter->physaddr, size, vma->vm_page_prot))
			return -ENODEV;
	...
	vma->vm_ops = &igb_mmap_ops;
        ...
        ptr[0] = adapter->addr;
        ptr[1] = (void *)adapter->physaddr;

	return 0;
}

/* rest is copied from igb lib */

int igb_setup_flex_filter(struct igb_adapter *adapter, unsigned int queue_id,
			  ...
			  u_int8_t *filter, u_int8_t *mask)
{
	...
	if (filter_len % 8) {
		...
		printk(KERN_WARNING "filter_len(%d) should be a 8 byte aligned value\n",
					filter_len);

		filter_buf = kmalloc(1, aligned_filter_len);
		if (!filter_buf)
			return -ENOMEM;
...
* add start filter of 6 bytes all 0xFF
	...
	E1000_WRITE_REG_ARRAY(hw, E1000_FHFT(filter_id),
			63, (1 << 24) | (queue_id << 8) | filter_len);  // 1 << 24 <= immediate interrupt enabled
...
int igb_clear_flex_filter(struct igb_adapter *adapter, unsigned int filter_id)
{
	struct e1000_hw *hw;
	u32 wufc;
